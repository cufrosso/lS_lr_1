...
Первоначально пользователь попадает на форму входа (signin.php). Если у него есть аккаунт, то он вводит логин, пароль и в случае корректного ввода перенаправляется на страницу профиля (profile.php). Если же он ввел либо логин, либо пароль неправильно, то возвращается на ту же страницу, с сообщением об ошибке.
